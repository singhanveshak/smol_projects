    ...
    with open(filename) as f:
        ...
        while(ch!=''): 
            ...
            while(flag==0):
                ...
                if(ch+f.read(1)+f.read(1)=='for'):      #IF A 'for' IS ENCOUNTERED  
                    for_mode=1                          #ACTIVATE FOR_MODE  
                f.seek(f.tell()-2)
                
                if(ch==';'):                            #IF ; IS ENCOUNTERED
                    if(for_mode==0):                    #WHILE FOR_MODE=0
                        flag=1
                ...
                if(ch=='{'):                            #IF { IS ENCOUNTERED
                    if(for_mode==0):                    
                        ...
                        for_mode=0                      #WHLIE FOR_MODE=1
                        f.seek(f.tell()-1)
                        flag=1

                if(ch=='}' or ch ==''):                 #IF } OR '' IS ENCOUNTERED
                    flag=1
                line+=ch
