...
for Type in glist:                    #for each group in glist. Each "Type" is a dictionary
  ...
  for plan in productList:            #each plan is a dictionary
    ...
    description=plan['description'].split(',')[0].replace(' ','_')
    full_description=plan['description'].split(',')[0].replace(' ','_')
    ...
    if(_data=='1' and _talk=='1' and _sms=='1'):
      ...
      condition=False
    f.write(str([goodness,data,sms,validity,price,full_description])+'\n')
    ...
    if(condition and producttype=='Recharge'):
      lst.append([goodness,data,sms,validity,price,full_description])
  
  df=pd.DataFrame(lst,columns=['Goodness','Data','SMS','Validity','Price(₹)','Talktime'])
  ...
  print('\n================================[   '+name+'   ]================================\n')
  if(not df.empty):
    print(df[['Data','SMS','Validity','Price(₹)','Talktime']])
print("==================================================================================")
